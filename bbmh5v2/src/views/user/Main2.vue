...
export default {
  ...
  data () {
    return {
      ...
      itemOrderList: { icon: 'icon-sell-order', name: '我的转卖订单', path: '/user/sellrec' },
      itemBank: { icon: 'icon-alipay', name: '收款账号', path: '/user/payaccs' },
  ...
  computed: {
    ...
    tipPayType () { // 收款账号
      // if (this.$store.state.user.userAccountInfo.wechatStatus == 3 || this.$store.state.user.userAccountInfo.alipayStatus == 3) {
      //   return '审核未通过'
      // } else if (this.$store.state.user.userAccountInfo.wechatStatus == 2 || this.$store.state.user.userAccountInfo.alipayStatus == 2) {
      //   return '待审核'
      // } else if (this.$store.state.user.userAccountInfo.wechatStatus == 1 || this.$store.state.user.userAccountInfo.alipayStatus == 1) {
      //   return '去查看'
      // }
      if (this.$store.state.user.userAccountInfo.accountStatus == 1) {
        return '去查看'
      }
      return '去完善'
    },
