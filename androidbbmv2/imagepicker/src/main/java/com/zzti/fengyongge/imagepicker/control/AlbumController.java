...
public class AlbumController {
    ...
    public List<PhotoModel> getCurrent() {
        Cursor cursor = resolver.query(Media.EXTERNAL_CONTENT_URI, new String[]{MediaColumns.DATA,MediaColumns._ID,
        ...
        cursor.moveToLast();
        do {
            if (cursor.getLong(cursor.getColumnIndex(MediaColumns.SIZE)) > 0) {//可以添加限制判断。多少byte的图片不让上传
                PhotoModel photoModel = new PhotoModel();
                photoModel.setOriginalPath(cursor.getString(cursor.getColumnIndex(MediaColumns.DATA)));
                if (new File(cursor.getString(cursor.getColumnIndex(MediaColumns.DATA))).exists()) {//文件不存在，不添加集合，防止有的图片黑
                    if (cursor.getString(cursor.getColumnIndex(MediaColumns.DATA)).contains("/DCIM/Camera")){
...
//                continue;
    ...
    public List<PhotoModel> getAlbum(String name) {
        ...
        cursor.moveToLast();
        do {
            if (cursor.getLong(cursor.getColumnIndex(MediaColumns.SIZE)) > 0) {
                PhotoModel photoModel = new PhotoModel();
                photoModel.setOriginalPath(cursor.getString(cursor.getColumnIndex(MediaColumns.DATA)));
                if (new File(cursor.getString(cursor.getColumnIndex(MediaColumns.DATA))).exists()) {//文件不存在，不添加集合，防止有的图片黑
                    photos.add(photoModel);
